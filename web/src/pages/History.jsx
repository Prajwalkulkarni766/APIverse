...
const History = () => {
  ...
  const getHistory = async (page = 1, reset = false) => {
    try {
      ...
      setHistoryData(reset ? response.data.history : [...historyData, ...response.data.history]);
  ...
  return (
    <div className="p-4 bg-white">
      ...
      <div className="flex gap-4 mb-6">
        ...
        <button
          className="px-4 py-2 text-[#FF6C37] hover:bg-red-50 rounded-md border border-red-500"
        ...
        <table className="w-full text-sm text-left text-gray-700">
          ...
          <tbody>
            ...
            {!loading && historyData.length === 0 && (
              <tr>
                <td colSpan="7" className="py-8 text-center text-gray-500">No content in table</td>
              </tr>
            )}
            {!loading &&
              historyData.map((item, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-3 font-medium">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold ${item.method === "GET"
                  ...
                  <td className="px-6 py-3 text-blue-500 truncate max-w-xs">{item.url}</td>
                  <td className="px-6 py-3">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold ${item.status === "SUCCESS"
                        ? "bg-green-100 text-green-600"
                        : item.status === "ERROR"
                          ? "bg-red-100 text-red-600"
